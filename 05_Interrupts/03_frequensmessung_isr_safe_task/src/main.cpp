...
{
  pulseCount++;
  BaseType_t xHigherPrio = pdFALSE;
  vTaskNotifyGiveFromISR(frequencyTaskHandle, &xHigherPrio);
  portYIELD_FROM_ISR();
...
void highPrio(void *pvParameters){
    while (true) {
    Serial.println("High Priority Task läuft!");
...
void setup()
{
  ...
  xTaskCreatePinnedToCore(highPrio, "High Prio", 3000, nullptr, 3, nullptr, espcore);
}
